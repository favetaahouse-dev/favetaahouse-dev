-- Harden mark_order_paid idempotency: only the genuine PENDING->PAID transition may run the
-- side effects. The previous guard blocklisted PAID/FULFILLED but let CANCELLED and REFUNDED
-- fall through, so a replayed webhook/return (SkipCash keeps returning statusId=2 forever)
-- could un-cancel/un-refund an order, double-decrement stock, double-bump coupon usage, and
-- resend the confirmation email. Transition-from-PENDING-only closes every one of those.
