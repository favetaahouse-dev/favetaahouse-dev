...
async function copyAssets() {
  ...
  await copyFirstMatch([SHOP_FILES], /^Logo_2-q2f906681\.png$/i, path.join(PUB, "brand", "logo-dark.png"))
    || await copyFirstMatch([SHOP_FILES], /^Logo_2-.*\.png$/i, path.join(PUB, "brand", "logo-dark.png"));
  ...
  // 5. videos (only the two saved locally: hero + campaign)
  const videosRoot = path.join(ASSETS, "www.alessiaabaya.com", "cdn", "shop", "videos");
  const hero = await findFile(path.join(videosRoot, "c", "vp", "ba60e5282cac4764996e3ebbfee61d65"), /\.mp4$/i);
  const campaign = await findFile(path.join(videosRoot, "c", "vp", "4e36a92247e14562ba64ee0f1eb8cc5f"), /\.mp4$/i);
  await fs.ensureDir(path.join(PUB, "video"));
  if (hero) await fs.copy(hero, path.join(PUB, "video", "hero.mp4"), { overwrite: true });
  if (campaign) await fs.copy(campaign, path.join(PUB, "video", "campaign.mp4"), { overwrite: true });
  console.log(`  ✓ videos (hero:${!!hero} campaign:${!!campaign})`);
}
