...
// Brute-force / credential-stuffing throttle: count recent FAILED attempts from this IP and
// block past the threshold. Keyed on IP (not the victim's email) to avoid a lockout-DoS
// against a targeted account. Distributed attacks still need platform-level (WAF) limiting.
...
async function tooManyFailures(ip: string | null): Promise<boolean> {
  ...
  try {
    ...
    return (count ?? 0) >= MAX_FAILED_PER_IP;
  } catch {
    return false; // fail open — never block a legit login on a counting-query error
  }
}
    ...
    Credentials({
      ...
      authorize: async (creds, req) => {
        ...
        if (await tooManyFailures(ip)) {
          // Do NOT log this blocked attempt as a failure: counting throttle-rejected requests
          // would let the block sustain itself and keep sliding the window forward purely from
          // being blocked. The counter now reflects only genuine bad credentials, so the block
          // self-heals a full window after the last real failure.
          return null;
        }
