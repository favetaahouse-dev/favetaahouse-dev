...
export async function getCollectionProducts(handle: string, q: CollectionQuery = {}, locale: string) {
  "use cache";
  cacheLife("hours");
  cacheTag("products");
  ...
  return {
    products: ((data ?? []) as Parameters<typeof toCard>[0][]).map((r) => toCard(r, locale)) as ProductCardDTO[],
    total: count ?? 0,
  };
}
...
export async function getCollectionTitle(handle: string, locale: string): Promise<string> {
  "use cache";
  cacheLife("days");
  cacheTag("content");
  if (locale === "ar") {
    // An admin-set Arabic collection title wins (matches the navbar), then the canonical
    // nav label; otherwise fall through to the English sources below (keep-English rule).
    const { data } = await supabase.from("collections").select("title_ar").eq("handle", handle).maybeSingle();
    if (data?.title_ar) return data.title_ar;
    const navKey = NAV_KEY[handle];
    if (navKey) {
