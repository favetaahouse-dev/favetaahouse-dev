...
export function IconButton({
  ...
  return (
    <button
      {...rest}
      className={cn(
        "inline-flex h-8 w-8 items-center justify-center text-secondary transition-colors hover:bg-elevated hover:text-foreground disabled:opacity-50",
        className,
      )}
    >
      {children}
...
export function Badge({ tone = "neutral", children }: { tone?: Tone; children: ReactNode }) {
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-medium uppercase tracking-[0.1em]",
        TONES[tone],
      )}
    >
      {children}
