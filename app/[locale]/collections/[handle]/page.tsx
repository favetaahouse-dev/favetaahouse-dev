...
export async function generateMetadata({
  ...
  const title = await getCollectionTitle(handle, locale);
  return { title };
}
...
export default async function CollectionPage({
  ...
  return (
    <div>
      {/* Tighter beneath than above: the title belongs to the toolbar and grid under it,
          not to the header above it. */}
      <div className="px-4 pt-10 pb-6 text-center md:px-8">
        <h1 className="section-title">{title}</h1>
      </div>
      <Suspense fallback={<CollectionSkeleton />}>
        <CollectionResults handle={handle} locale={locale} searchParams={searchParams} />
      </Suspense>
    </div>
  );
}
